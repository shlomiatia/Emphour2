...
static func create_offer(group: String, loyalty: int, deck: Array, crossbowman_unlocked := true) -> Dictionary:
...
static func create_final_offer(group: String, upgrade: bool, deck: Array) -> Dictionary:
...
static func pick_final_old(deck: Array) -> String:
    var choices := deck.filter(func(entry: Variant) -> bool: return TIERS[1].has(card_name(entry)))
    return "" if choices.is_empty() else card_name(choices.pick_random())

static func pick_old_card(group: String, reward: Dictionary, deck: Array, crossbowman_unlocked: bool) -> String:
    if !reward["upgrade"]:
        return ""
    ...
    for tier in tiers:
        if tier < reward["from"] || tier >= reward["tier"]:
            continue
        var choices := deck.filter(func(entry: Variant) -> bool: return TIERS[tier].has(card_name(entry)) && !get_upgrade_cards(group, card_name(entry), reward["tier"], crossbowman_unlocked).is_empty())
        if !choices.is_empty():
