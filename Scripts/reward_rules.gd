...
static func get_reward(group: String, loyalty: int) -> Dictionary:
    ...
    return {"upgrade": parts[0] == "upgrade", "from": float(parts[1]) if parts.size() == 3 else 0.0, "tier": float(parts[-1])}
...
static func get_upgrade_cards(group: String, card_name: String, tier: float) -> Array[String]:
    var upgrade := get_data().get_value("upgrades", card_name, "") as String
    var cards: Array[String]
    cards.assign(get_tier_cards(tier))
    return cards.filter(func(card: String) -> bool: return belongs_to(card, group) && (upgrade == "all" || upgrade.split(",").has(card)))

static func get_group_cards(group: String, tier: float) -> Array[String]:
    var choices: Array[String]
    choices.assign(get_tier_cards(tier).filter(func(card: String) -> bool: return belongs_to(card, group)))
    return choices

static func get_tier_cards(tier: float) -> Array:
    var key: Variant = int(tier) if is_equal_approx(tier, roundf(tier)) else tier
    return TIERS[key]
